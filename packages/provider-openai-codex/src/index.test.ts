...
describe("CodexCliProvider", () => {
  ...
  it("requires every Codex Word operation schema property for strict structured output", () => {
    const operationSchema =
      codexOutputSchema.properties.wordChangesets.items.properties.operations.items;

    expect(operationSchema.required).toEqual(Object.keys(operationSchema.properties));
    expect(operationSchema.properties.blockId.type).toEqual(["string", "null"]);
    expect(operationSchema.properties.block.type).toEqual(["object", "null"]);
  });
  ...
  it("turns Codex Word edit output into a reviewable Word changeset", async () => {
    ...
    const provider = new CodexCliProvider({
      runCodexExec: (request) => {
        ...
        } as unknown as CodexAgentResponse);
      }
    });
    ...
    expect(providerPrompt).toContain("Active Word paragraph blocks:");
    expect(providerPrompt).toContain("id=w1");
    expect(providerPrompt).toContain("id=w2");
    expect(providerPrompt).toContain("afterBlockId, block, startOffset");
  });

  it("includes headings and tables from Word document structure in the prompt", async () => {
    let providerPrompt = "";
    const provider = new CodexCliProvider({
      runCodexExec: (request) => {
        providerPrompt = request.prompt;
        return Promise.resolve(createCodexAnswer("Reviewed the document."));
      }
    });
    ...
    await provider.startSession(
      {
        ...
        activeFilePath: "report.docx",
        activeDocument: {
          kind: "word",
          path: "report.docx",
          plainText: "Executive Summary\n\nName Score\nAda 97",
          blocks: [
            { id: "w1", kind: "paragraph", text: "Executive Summary" },
            { id: "w2", kind: "paragraph", text: "Name Score Ada 97" }
          ],
          warnings: [],
          structure: [
            {
            ...
            },
            {
              ...
              columnCount: 2,
              cells: [
                { id: "tbl-1-r0c0-ccc", rowIndex: 0, columnIndex: 0, text: "Name" },
                { id: "tbl-1-r0c1-ddd", rowIndex: 0, columnIndex: 1, text: "Score" },
                { id: "tbl-1-r1c0-eee", rowIndex: 1, columnIndex: 0, text: "Ada" },
                { id: "tbl-1-r1c1-fff", rowIndex: 1, columnIndex: 1, text: "97" }
              ]
            }
          ],
          structureWarnings: []
    ...
    expect(providerPrompt).toContain("Document structure (headings and tables");
    expect(providerPrompt).toContain('Heading (level 1): "Executive Summary"');
    expect(providerPrompt).toContain("Table tbl-1-bbb (2 rows x 2 columns):");
    expect(providerPrompt).toContain('R0C0="Name"  R0C1="Score"');
    expect(providerPrompt).toContain('R1C0="Ada"  R1C1="97"');
  });

  it("turns Codex table-edit output into a reviewable table changeset", async () => {
    const wordBlocks = [{ id: "w1", kind: "paragraph" as const, text: "Report" }];
    const provider = new CodexCliProvider({
      runCodexExec: () =>
        Promise.resolve({
          action: "word-edit",
          targetFilePath: "report.docx",
          summary: "Update score cell",
          afterContents: "",
          patches: [],
          wordChangesets: [
            {
          ...
          message: "I updated the score cell.",
          notes: "Generated by fake Codex runner"
        } as unknown as CodexAgentResponse)
    });
    ...
    const result = await provider.startSession(
      {
        ...
        mode: "apply-with-review",
        projectRoot: "/tmp/project",
        prompt: "Update Ada's score to 100.",
        activeFilePath: "report.docx",
        activeDocument: {
          kind: "word",
          path: "report.docx",
  ...
  it("retries schema failures with focused Word-edit instructions", async () => {
    const calls: string[] = [];
    const prompts: string[] = [];
    const wordBlocks = [
      { id: "w1", kind: "paragraph" as const, text: "Draft placeholder." }
    ];
    const provider = new CodexCliProvider({
      runCodexExec: (request) => {
        ...
        if (prompts.length === 1) {
          return Promise.reject(
            new Error("Codex output did not match the expected agent response schema.")
          );
        }

        return Promise.resolve({
          action: "word-edit",
          targetFilePath: "brief.docx",
          summary: "Insert briefing",
          afterContents: "",
          patches: [],
          wordChangesets: [
            {
              ...
              operations: [
                {
                  type: "replace-block",
                  blockId: "w1",
                  afterText: "Executive briefing content.",
                  afterBlockId: null,
                  block: null,
                  startOffset: null,
                  endOffset: null,
          ...
          message: "I prepared a Word changeset.",
          notes: "Retry produced valid Word edit"
        } as unknown as CodexAgentResponse);
      }
    });
    ...
    const result = await provider.startSession(
      {
        ...
        mode: "apply-with-review",
        projectRoot: "/tmp/project",
        prompt: "Do a web search and put findings in the Word document.",
        activeFilePath: "brief.docx",
        activeDocument: {
          kind: "word",
          path: "brief.docx",
          plainText: "Draft placeholder.",
          blocks: wordBlocks,
          warnings: []
        },
        compiler: "pdflatex"
    ...
    expect(prompts[1]).toContain("Failed attempt:");
    expect(prompts[1]).toContain(
      'Use action "patch" for a minimal source edit, "word-edit"'
    );
    expect(prompts[1]).toContain("Active Word paragraph blocks:");
    expect(prompts[1]).toContain("id=w1");
    expect(
      result.events.some(
        (event) =>
  ...
  it("retries section-merge requests into a concrete source edit", async () => {
    ...
    expect(prompts[0]).toContain("Write message like a person reporting back");
  ...
  it("runs autonomous Codex sessions with direct project write access", async () => {
    ...
    expect(providerPrompt).toContain("full project-scoped access");
    ...
    expect(providerPrompt).toContain(
      "If a search command, regex, glob, or shell command fails"
    );
  ...
  it("retries interrupted Codex planner runs once with focused context", async () => {
    ...
    expect(prompts).toHaveLength(2);
    expect(prompts[1]).toContain(
      "previous Codex planner attempt failed before it returned a valid action"
    );
  ...
  it("does not retry non-transient Codex planner failures", async () => {
    ...
    const provider = new CodexCliProvider({
      runCodexExec: (request) => {
        prompts.push(request.prompt);
        throw new Error("Codex CLI crashed with an unexpected provider bug.");
      }
    });
    ...
    expect(
      result.events.some(
        (event) =>
          ...
          event.content.includes("I ran the compile, and it passed.") &&
          !event.content.includes("Build details are available in the Log panel.") &&
          !event.content.includes("Diagnostics:") &&
          !event.content.includes("Compile succeeded.")
      )
    ).toBe(true);
